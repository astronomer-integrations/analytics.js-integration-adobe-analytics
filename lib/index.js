...
AdobeAnalytics
  ...
  .tag(
    'default',
    '<script src="//cdn.metarouter.io/a/assets/adobe/appmeasurement.js">'
  )
  .tag(
    'heartbeat',
    '<script src="//cdn.metarouter.io/a/assets/adobe/appmeasurement-heartbeat.js">'
  );
...
AdobeAnalytics.prototype.processEvent = function(msg, adobeEvent) {
  ...
  if (Array.isArray(products) && !this.s.products) {
  ...
  if (productVariables) update(productVariables, 'products', this.s); //eslint-disable-line
...
function aliasEvent(name, mapping) {
  ...
  if (mapping) {
    for (var k in mapping) {
      if (mapping.hasOwnProperty(k) && key === k.toLowerCase()) {
        events.push(mapping[k]);
      }
    }
  }
